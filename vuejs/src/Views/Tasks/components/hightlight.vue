<template>
  <div class="position-absolute cursor-pointer" @click="AlertSelectedText" :style="{top: offsetTop, left: offsetLeft}" ref="popup" v-show="selected">
  ...
  methods: {
    ...
    AlertSelectedText() {
     // alert(`"${this.selectedText}" selected`);
      this.$emit("selectedData",this.selectedText)
    }
  },
  mounted() {
