	    ...
	    */
        ...
        getTasks = async (cr_user, body, res) => {
               ...
               if(page > 0){
               	offset = (page - 1) * limit;
					...
					let projects = await TaskModal.findAndCountAll({
		...
		getTaskById = async (cruser, body, res) => {
				...
				try {
					...
					const project = await TaskModal.findOne({
			            where: {id},
			            include: 'created_by_user' 
			        });
		...
		AddEditTask = async (cruser, body, res) => {
			...
			try {
				...
				const prdatareturn = await TaskModal.create(data).then(async prData => {
												    return prData;
												}).catch((error) => {
		...
		updateTask = async (id,body, res) => {
				...
				try {
					...
					const prdatareturn = await TaskModal.findOne({
											            where: {id: id}
											         }).then(async prData => {
